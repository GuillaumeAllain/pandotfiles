...
def tikzyamlparse(file_path):
    ...
    try:
        ...
        raise FileNotFoundError(
            "No configuration file found in " + file_path
        ) from fnofe
    ...
    if "files" in yaml_config.keys():
        ...
        if "src_folder" not in yaml_config.keys():
            warnings.warn(
                "No explicit source folder defined for tikz file, using 'tikz'"
            )
            yaml_config["src_folder"] = "tikz"
        warnings.warn(warning_string)
        try:
            yaml_config["files"] = [
                yaml_config["src_folder"] + "/" + x
                for x in listdir(yaml_config["src_folder"])
                if splitext(x)[1] == ".tikz"
            ]
            yaml_config["files_makestring"] = (
                "$(wildcard " + yaml_config["src_folder"] + "/*.tikz)"
            )
