...
def generate_latex_file(data_dic):
    ...
    LATEX_FILE_HEADER += "%---------------------------" "\n"
    LATEX_FILE_HEADER += rf"\documentclass[{data_dic['font_size']}]{{article}}" "\n"
    if "font" in data_dic.keys():
        LATEX_FILE_HEADER += r"\usepackage{fontspec}" "\n"
        LATEX_FILE_HEADER += rf"\setmainfont{{{data_dic['font']}}}" "\n"
    LATEX_FILE_HEADER += r"\usepackage{tikz}""\n"
    LATEX_FILE_HEADER += r"\usetikzlibrary{external}""\n"
    LATEX_FILE_HEADER += (
